...
import functionSet.Sqrt;



public class FSProblemFinalMulti extends GPProblem {
	...
	private GPConfiguration config;
	 ...
	 public static String[][] GPselectedfeatures;
  ...
  // show results for all generations
	 ...
	 public FSProblemFinalMulti(GPConfiguration conf, double[][] inst, double[] out) throws InvalidConfigurationException {
	  
      this.config = conf;		               
      FSProblemFinalMulti.instances = inst;
      FSProblemFinalMulti.output = out;
	 ...
	 public GPGenotype create() throws InvalidConfigurationException {
	  ...
	  // Arguments of result-producing chromosome: none
	    ...
	    FSProblemFinalMulti.features = new Variable[FSProblemFinalMulti.instances[0].length-1];		   
		   
		for(int j=0; j < FSProblemFinalMulti.features.length; j++) 
		    {
			FSProblemFinalMulti.features[j] = Variable.create(config, "Feature-"+(j+1), CommandGene.DoubleClass); 
		    } 
	    
	    
	   CommandGene[] vars = new CommandGene[FSProblemFinalMulti.instances[0].length-1];
	   for(int j=0; j < FSProblemFinalMulti.instances[0].length-1; j++) {
	                   vars[j] = FSProblemFinalMulti.features[j];
	                }
	   CommandGene[] funcs = {
		    			    ...
		    			    new Sqrt(config, CommandGene.DoubleClass),
		    			   // new Sine(config, CommandGene.DoubleClass),
	                       //  new Tangent(config, CommandGene.DoubleClass),
		                ...
		                GPGenotype result = GPGenotype.randomInitialGenotype(config, types, argTypes, nodeSets, 50, true); // 20 = maxNodes, true = verbose output
	 ...
	 public static class FSFitnessFunction extends GPFitnessFunction {
	      ...
	      // needed in evaluate
	        ...
	        double totalCorrect = 0.0d;		
	         ...
	         // penalty if the terms are less than minNodes
	            if (FSProblemFinalMulti.minNodes >= 0) {
	                if (numTerms < minNodes) {
	                	FSProblemFinalMulti.penalty += Math.abs(numTerms - minNodes)*FSProblemFinalMulti.minNodesPenalty; 
	                }
	            }
	        	   
	         // for all different type of constraints (Note: This could surely be done more efficient.)
	            if (FSProblemFinalMulti.alldifferentVariables) {
        ...
        public static void myOutputSolution(IGPProgram a_best, int gen, int numEvolutions)  {
		       ...
		       System.out.println(". \nNumber of functions + terminals: " + numTerms + " (" + numFunctions + " functions, " + numTerminals + " terminals)");
