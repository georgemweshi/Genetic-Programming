...
public class FSProblemFinal2 extends GPProblem {
  ...
  // show results for all generations
	 ...
	 public FSProblemFinal2(GPConfiguration conf, double[][] inst, double[] out) throws InvalidConfigurationException {
	  
      this.config = conf;		               
      FSProblemFinal2.instances = inst;
      FSProblemFinal2.output = out;
	 ...
	 public GPGenotype create() throws InvalidConfigurationException {
	  ...
	  // Arguments of result-producing chromosome: none
	    ...
	    FSProblemFinal2.features = new Variable[FSProblemFinal2.instances[0].length-1];		   
		   
		for(int j=0; j < FSProblemFinal2.features.length; j++) 
		    {
			FSProblemFinal2.features[j] = Variable.create(config, "Feature-"+(j+1), CommandGene.DoubleClass); 
		    } 
	    
	    
	   CommandGene[] vars = new CommandGene[FSProblemFinal2.instances[0].length-1];
	   for(int j=0; j < FSProblemFinal2.instances[0].length-1; j++) {
	                   vars[j] = FSProblemFinal2.features[j];
	                }
	   CommandGene[] funcs = {
		                ...
		                GPGenotype result = GPGenotype.randomInitialGenotype(config, types, argTypes, nodeSets, 300, true); // 20 = maxNodes, true = verbose output
	      ...
	      // needed in evaluate
	        ...
	        double totalCorrect = 0.0d;		
	         ...
	         // penalty if the terms are less than minNodes
	            if (FSProblemFinal2.minNodes >= 0) {
	                if (numTerms < minNodes) {
	                	FSProblemFinal2.penalty += Math.abs(numTerms - minNodes)*FSProblemFinal2.minNodesPenalty; 
	                }
	            }
	        	   
	         // for all different type of constraints (Note: This could surely be done more efficient.)
	            if (FSProblemFinal2.alldifferentVariables) {
